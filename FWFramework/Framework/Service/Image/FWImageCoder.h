 ...
 */
typedef NS_ENUM(NSUInteger, FWImageBlendOperation) {
 ...
 */
@interface FWImageFrame : NSObject <NSCopying>
...
@property (nonatomic) FWImageDisposeMethod dispose;     ///< Frame dispose method.
@property (nonatomic) FWImageBlendOperation blend;      ///< Frame blend operation.
...
/**
 ...
 Example:
    ...
    FWImageDecoder *decoder = [FWImageDecoder decoderWithData:data scale:2.0];
    UIImage image = [decoder frameAtIndex:0 decodeForDisplay:YES].image;
 
    // Decode image during download:
    NSMutableData *data = [NSMutableData new];
    FWImageDecoder *decoder = [[FWImageDecoder alloc] initWithScale:2.0];
 ...
 */
@interface FWImageDecoder : NSObject

@property (nullable, nonatomic, readonly) NSData *data;    ///< Image data.
@property (nonatomic, readonly) FWImageType type;          ///< Image data type.
 ...
 */
- (nullable FWImageFrame *)frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay;
...
/**
 ...
 @discussion It supports encoding single frame image with the type defined in FWImageType.
 It also supports encoding multi-frame image with GIF, APNG and WebP.
 
 Example:
    
    FWImageEncoder *jpegEncoder = [[FWImageEncoder alloc] initWithType:FWImageTypeJPEG];
    jpegEncoder.quality = 0.9;
    [jpegEncoder addImage:image duration:0];
    NSData jpegData = [jpegEncoder encode];
 
    FWImageEncoder *gifEncoder = [[FWImageEncoder alloc] initWithType:FWImageTypeGIF];
 ...
 */
@interface FWImageEncoder : NSObject

@property (nonatomic, readonly) FWImageType type; ///< Image type.
 ...
 */
- (nullable instancetype)initWithType:(FWImageType)type NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 @return The image data, or nil if an error occurs.
 */
+ (nullable NSData *)encodeImage:(UIImage *)image type:(FWImageType)type quality:(CGFloat)quality;
...
/**
 ...
 @return The image data, or nil if an error occurs.
 */
+ (nullable NSData *)encodeImageWithDecoder:(FWImageDecoder *)decoder type:(FWImageType)type quality:(CGFloat)quality;
...
@interface UIImage (FWImageCoder)
 ...
 */
@property (nonatomic) BOOL fwIsDecodedForDisplay;
 ...
 */
- (void)fwSaveToAlbumWithCompletionBlock:(nullable void(^)(NSURL * _Nullable assetURL, NSError * _Nullable error))completionBlock;
 ...
 */
- (nullable NSData *)fwImageDataRepresentation;
...
CG_EXTERN FWImageType FWImageDetectType(CFDataRef data);

/// Convert FWImageType to UTI (such as kUTTypeJPEG).
CG_EXTERN CFStringRef _Nullable FWImageTypeToUTType(FWImageType type);

/// Convert UTI (such as kUTTypeJPEG) to FWImageType.
CG_EXTERN FWImageType FWImageTypeFromUTType(CFStringRef uti);

/// Get image type's file extension (such as @"jpg").
CG_EXTERN NSString *_Nullable FWImageTypeGetExtension(FWImageType type);
...
CG_EXTERN BOOL FWCGColorSpaceIsDeviceGray(CGColorSpaceRef space);



/// Convert EXIF orientation value to UIImageOrientation.
CG_EXTERN UIImageOrientation FWUIImageOrientationFromEXIFValue(NSInteger value);

/// Convert UIImageOrientation to EXIF orientation value.
CG_EXTERN NSInteger FWUIImageOrientationToEXIFValue(UIImageOrientation orientation);
 ...
 */
CG_EXTERN CGImageRef _Nullable FWCGImageCreateDecodedCopy(CGImageRef imageRef, BOOL decodeForDisplay);
 ...
 */
CG_EXTERN CGImageRef _Nullable FWCGImageCreateCopyWithOrientation(CGImageRef imageRef,
...
/**
 ...
 @return A new image, or NULL if an error occurs.
 */
CG_EXTERN CGImageRef _Nullable FWCGImageCreateAffineTransformCopy(CGImageRef imageRef,
 ...
 */
CG_EXTERN CFDataRef _Nullable FWCGImageCreateEncodedData(CGImageRef imageRef, FWImageType type, CGFloat quality);
 ...
 */
CG_EXTERN NSUInteger FWImageGetWebPFrameCount(CFDataRef webpData);
 ...
 */
CG_EXTERN CGImageRef _Nullable FWCGImageCreateWithWebPData(CFDataRef webpData,
                                                           BOOL decodeForDisplay,
                                                           BOOL useThreads,
                                                           BOOL bypassFiltering,
                                                           BOOL noFancyUpsampling);

typedef NS_ENUM(NSUInteger, FWImagePreset) {
    FWImagePresetDefault = 0,  ///< default preset.
    FWImagePresetPicture,      ///< digital picture, like portrait, inner shot
    FWImagePresetPhoto,        ///< outdoor photograph, with natural lighting
    FWImagePresetDrawing,      ///< hand or line drawing, with high-contrast details
    FWImagePresetIcon,         ///< small-sized colorful images
    FWImagePresetText          ///< text-like
};
...
/**
 ...
 @param preset        Preset for different image type, default is FWImagePresetDefault.
 @return WebP data, or nil if an error occurs.
 */
CG_EXTERN CFDataRef _Nullable FWCGImageCreateEncodedWebPData(CGImageRef imageRef,
                                                             BOOL lossless,
                                                             CGFloat quality,
                                                             int compressLevel,
                                                             FWImagePreset preset);
