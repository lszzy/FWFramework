...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    [db makeFunctionNamed:@"RemoveDiacritics" arguments:1 block:^(void *context, int argc, void **argv) {
        FWSqliteValueType type = [self.db valueType:argv[0]];
        if (type == FWSqliteValueTypeNull) {
            [self.db resultNullInContext:context];
             return;
        }
        if (type != FWSqliteValueTypeText) {
...
typedef NS_ENUM(int, FWSqliteValueType) {
    FWSqliteValueTypeInteger = 1,
    FWSqliteValueTypeFloat   = 2,
    FWSqliteValueTypeText    = 3,
    FWSqliteValueTypeBlob    = 4,
    FWSqliteValueTypeNull    = 5
};

- (FWSqliteValueType)valueType:(void *)argv;
...
@interface FWDatabase (FWDatabaseAdditions)
...
@interface FWDatabasePool : NSObject
...
- (BOOL)databasePool:(FWDatabasePool*)pool shouldAddDatabaseToPool:(FWDatabase*)database;
