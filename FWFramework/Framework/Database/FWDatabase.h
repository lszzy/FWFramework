 ...
 */
typedef int(^FWDBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);

/**
 Enumeration used in checkpoint methods.
 */
typedef NS_ENUM(int, FWDBCheckpointMode) {
    FWDBCheckpointModePassive  = 0, // SQLITE_CHECKPOINT_PASSIVE,
    FWDBCheckpointModeFull     = 1, // SQLITE_CHECKPOINT_FULL,
    FWDBCheckpointModeRestart  = 2, // SQLITE_CHECKPOINT_RESTART,
    FWDBCheckpointModeTruncate = 3  // SQLITE_CHECKPOINT_TRUNCATE
};
...
/** A SQLite ([https://sqlite.org/](https://sqlite.org/)) Objective-C wrapper.
 ...
 The three main classes in FWDB are:

 - @c FWDatabase - Represents a single SQLite database.  Used for executing SQL statements.

 - @c FWResultSet - Represents the results of executing a query on an @c FWDatabase .

 - @c FWDatabaseQueue - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 See also

 - @c FWDatabasePool - A pool of @c FWDatabase objects

 - @c FWStatement - A wrapper for @c sqlite_stmt

 External links

 - [FWDB on GitHub](https://github.com/ccgus/fmdb) including introductory documentation
 - [SQLite web site](https://sqlite.org/)
 - [FWDB mailing list](http://groups.google.com/group/fmdb)
 - [SQLite FAQ](https://sqlite.org/faq.html)

 @warning Do not instantiate a single @c FWDatabase  object and use it across multiple threads. Instead, use @c FWDatabaseQueue .
...
/** Create a @c FWDatabase  object.
 
 An @c FWDatabase  is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.

 2. An zero-length string.  An empty database is created at a temporary location.  This database is deleted with the @c FWDatabase  connection is closed.

 3. @c nil .  An in-memory database is created.  This database will be destroyed with the @c FWDatabase  connection is closed.
...
FWDatabase *db = [FWDatabase databaseWithPath:fileURL.path];
@endcode
 ...
 @return @c FWDatabase  object if successful; @c nil  if failure.
...
/** Create a @c FWDatabase  object.
 ...
 2. @c nil .  An in-memory database is created.  This database will be destroyed with the @c FWDatabase  connection is closed.
...
NSURL *folder  = [[NSFileManager defaultManager] URLForDirectory:NSApplicationSupportDirectory inDomain:NSUserDomainMask appropriateForURL:nil create:true error:&error];
NSURL *fileURL = [folder URLByAppendingPathComponent:@"test.db"];
FWDatabase *db = [FWDatabase databaseWithURL:fileURL];
@endcode
...
/** Initialize a @c FWDatabase  object.
 
 An @c FWDatabase  is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.

 2. A zero-length string.  An empty database is created at a temporary location.  This database is deleted with the @c FWDatabase  connection is closed.
 ...
 NSURL *folder  = [[NSFileManager defaultManager] URLForDirectory:NSApplicationSupportDirectory inDomain:NSUserDomainMask appropriateForURL:nil create:true error:&error];
 NSURL *fileURL = [folder URLByAppendingPathComponent:@"test.db"];
 FWDatabase *db = [[FWDatabase alloc] initWithPath:fileURL.path];
 @endcode
...
/** Initialize a @c FWDatabase  object.
 ...
 An @c FWDatabase  is created with a local file URL to a SQLite database file.  This path can be one of these three:
 ...
 NSURL *folder  = [[NSFileManager defaultManager] URLForDirectory:NSApplicationSupportDirectory inDomain:NSUserDomainMask appropriateForURL:nil create:true error:&error];
 NSURL *fileURL = [folder URLByAppendingPathComponent:@"test.db"];
 FWDatabase *db = [[FWDatabase alloc] initWithURL:fileURL];
 @endcode
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(__attribute__((noescape)) FWDBExecuteStatementsCallbackBlock _Nullable)block;
...
/** Execute select statement
 ...
 Executing queries returns an @c FWResultSet  object if successful, and @c nil  upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the @c lastErrorMessage  and @c lastErrorMessage  methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[FWResultSet next]>`) from one record to the other.
 ...
 @return A @c FWResultSet  for the result set upon success; @c nil  upon failure. If failed, you can call @c lastError , @c lastErrorCode , or @c lastErrorMessage  for diagnostic information regarding the failure.
 
 @see FWResultSet
 @see [`FWResultSet next`](<[FWResultSet next]>)
 @see [`sqlite3_bind`](https://sqlite.org/c3ref/bind_blob.html)
...
/** Execute select statement
 ...
 `func executeQuery(sql: String, values: [Any]?) throws  -> FWResultSet!`
...
- (FWResultSet *)prepare:(NSString *)sql;
...
/// The FWDB version number as a string in the form of @c "2.7.7" .
///
/// If you want to compare version number strings, you can use NSNumericSearch option:
///
/// @code
/// NSComparisonResult result = [[FWDatabase FWDBUserVersion] compare:@"2.11.0" options:NSNumericSearch];
/// @endcode
///
...
FWResultSet *rs = [db executeQuery:@"SELECT * FROM employees WHERE RemoveDiacritics(first_name) LIKE 'jose'"];
...
@code
myDB.dateFormat = [FWDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
@endcode
 ...
 @warning Note that @c NSDateFormatter  is not thread-safe, so the formatter generated by this method should be assigned to only one FWDB instance and should not be used for other purposes.
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using @c FWDatabase:storeableDateFormat .
 ...
 @warning Note there is no direct getter for the @c NSDateFormatter , and you should not use the formatter you pass to FWDB for other purposes, as @c NSDateFormatter  is not thread-safe.
 */
...
/** Objective-C wrapper for @c sqlite3_stmt
 ...
 This is a wrapper for a SQLite @c sqlite3_stmt . Generally when using FWDB you will not need to interact directly with `FWStatement`, but rather with @c FWDatabase  and @c FWResultSet  only.
