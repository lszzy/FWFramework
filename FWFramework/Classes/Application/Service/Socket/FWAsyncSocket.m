...
NSString *const FWAsyncSocketException = @"FWAsyncSocketException";
NSString *const FWAsyncSocketErrorDomain = @"FWAsyncSocketErrorDomain";

NSString *const FWAsyncSocketQueueName = @"FWAsyncSocket";
NSString *const FWAsyncSocketThreadName = @"FWAsyncSocket-CFStream";

NSString *const FWAsyncSocketManuallyEvaluateTrust = @"FWAsyncSocketManuallyEvaluateTrust";
#if TARGET_OS_IPHONE
NSString *const FWAsyncSocketUseCFStreamForTLS = @"FWAsyncSocketUseCFStreamForTLS";
#endif
NSString *const FWAsyncSocketSSLPeerID = @"FWAsyncSocketSSLPeerID";
NSString *const FWAsyncSocketSSLProtocolVersionMin = @"FWAsyncSocketSSLProtocolVersionMin";
NSString *const FWAsyncSocketSSLProtocolVersionMax = @"FWAsyncSocketSSLProtocolVersionMax";
...
@interface FWAsyncSocketPreBuffer : NSObject
{
	uint8_t *preBuffer;
...
/**
 * The FWAsyncReadPacket encompasses the instructions for any given read.
 * The content of a read packet allows the code to determine if we're:
 *  - reading to a certain length
 *  - reading to a certain separator
 *  - or simply reading the first chunk of available data
**/
@interface FWAsyncReadPacket : NSObject
{
  @public
...
- (NSUInteger)readLengthForTermWithPreBuffer:(FWAsyncSocketPreBuffer *)preBuffer found:(BOOL *)foundPtr;
...
/**
 * The FWAsyncWritePacket encompasses the instructions for any given write.
**/
@interface FWAsyncWritePacket : NSObject
{
  @public
	NSData *buffer;
	NSUInteger bytesDone;
...
/**
 * The FWAsyncSpecialPacket encompasses special instructions for interruptions in the read/write queues.
 * This class my be altered to support more than just TLS in the future.
**/
@interface FWAsyncSpecialPacket : NSObject
{
  @public
...
@implementation FWAsyncSocket
{
	uint32_t flags;
	uint16_t config;
	
	__weak id<FWAsyncSocketDelegate> delegate;
	...
	FWAsyncReadPacket *currentRead;
	FWAsyncWritePacket *currentWrite;
	
	unsigned long socketFDBytesAvailable;
	
	FWAsyncSocketPreBuffer *preBuffer;
	...
	FWAsyncSocketPreBuffer *sslPreBuffer;
...
- (instancetype)initWithDelegate:(id<FWAsyncSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	if((self = [super init]))
	{
		...
		{
			socketQueue = dispatch_queue_create([FWAsyncSocketQueueName UTF8String], NULL);
		}
		...
		preBuffer = [[FWAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
- (void)dealloc
{
	LogInfo(@"%@ - %@ (start)", NSStringFromSelector(_cmd), self);
...
+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<FWAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq error:(NSError**)error {
  return [self socketFromConnectedSocketFD:socketFD delegate:aDelegate delegateQueue:dq socketQueue:NULL error:error];
}

+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<FWAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq error:(NSError* __autoreleasing *)error
{
  __block BOOL errorOccured = NO;
  
  FWAsyncSocket *socket = [[[self class] alloc] initWithDelegate:aDelegate delegateQueue:dq socketQueue:sq];
  ...
  dispatch_sync(socket->socketQueue, ^{ @autoreleasepool {
    ...
    if (retVal)
    {
      NSString *errMsg = NSLocalizedStringWithDefaultValue(@"FWAsyncSocketOtherError",
      ...
      errorOccured = YES;
      if (error)
        *error = [NSError errorWithDomain:FWAsyncSocketErrorDomain code:FWAsyncSocketOtherError userInfo:userInfo];
      return;
    }
...
- (void)getDelegate:(id<FWAsyncSocketDelegate> *)delegatePtr delegateQueue:(dispatch_queue_t *)delegateQueuePtr
{
	if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey))
	{
...
- (BOOL)doAccept:(int)parentSocketFD
{
	...
	if (delegateQueue)
	{
		...
		dispatch_async(delegateQueue, ^{ @autoreleasepool {
			...
			// Create FWAsyncSocket instance for accepted socket
			
			FWAsyncSocket *acceptedSocket = [[[self class] alloc] initWithDelegate:theDelegate
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"FWAsyncSocketConnectTimeoutError",
	                                                     @"FWAsyncSocket", [NSBundle mainBundle],
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"FWAsyncSocketReadMaxedOutError",
														 @"FWAsyncSocket", [NSBundle mainBundle],
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"FWAsyncSocketReadTimeoutError",
	                                                     @"FWAsyncSocket", [NSBundle mainBundle],
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"FWAsyncSocketWriteTimeoutError",
	                                                     @"FWAsyncSocket", [NSBundle mainBundle],
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"FWAsyncSocketClosedError",
	                                                     @"FWAsyncSocket", [NSBundle mainBundle],
...
- (BOOL)usingCFStreamForTLS
{
	...
	{
		// The startTLS method was given the FWAsyncSocketUseCFStreamForTLS flag.
                        ...
                        tag:(long)tag
{
	...
	FWAsyncReadPacket *packet = [[FWAsyncReadPacket alloc] initWithData:buffer
	                                                          startOffset:offset
	                                                            maxLength:length
	                                                              timeout:timeout
	                                                           readLength:0
...
- (float)progressOfReadReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
        ...
        if (!self->currentRead || ![self->currentRead isKindOfClass:[FWAsyncReadPacket class]])
		{
...
- (void)maybeDequeueRead
{
	...
	if ((currentRead == nil) && (flags & kConnected))
	{
		if ([readQueue count] > 0)
		{
			...
			if ([currentRead isKindOfClass:[FWAsyncSpecialPacket class]])
			{
				LogVerbose(@"Dequeued FWAsyncSpecialPacket");
			...
			{
				LogVerbose(@"Dequeued FWAsyncReadPacket");
...
- (void)flushSSLBuffers
{
	...
	if ([self usingCFStreamForTLS])
	{
		...
		{
			LogVerbose(@"%@ - Flushing ssl buffers into prebuffer...", NSStringFromSelector(_cmd));
			...
			LogVerbose(@"%@ - CFReadStreamRead(): result = %i", NSStringFromSelector(_cmd), (int)result);
	...
	if (estimatedBytesAvailable > 0)
	{
		...
		do
		{
			LogVerbose(@"%@ - estimatedBytesAvailable = %lu", NSStringFromSelector(_cmd), (unsigned long)estimatedBytesAvailable);
			...
			LogVerbose(@"%@ - read from secure socket = %u", NSStringFromSelector(_cmd), (unsigned)bytesRead);
			...
			LogVerbose(@"%@ - prebuffer.length = %zu", NSStringFromSelector(_cmd), [preBuffer availableBytes]);
...
- (void)doReadData
{
	...
	if ([self usingCFStreamForTLS])
	{
		...
		// Requested CFStream, rather than SecureTransport, for TLS (via FWAsyncSocketUseCFStreamForTLS)
...
- (void)completeCurrentRead
{
	...
	{
		FWAsyncReadPacket *theRead = currentRead; // Ensure currentRead retained since result may not own buffer
...
- (void)doReadTimeout
{
	...
	{
		FWAsyncReadPacket *theRead = currentRead;
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	FWAsyncWritePacket *packet = [[FWAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (float)progressOfWriteReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
        ...
        if (!self->currentWrite || ![self->currentWrite isKindOfClass:[FWAsyncWritePacket class]])
		{
...
- (void)maybeDequeueWrite
{
	...
	if ((currentWrite == nil) && (flags & kConnected))
	{
		if ([writeQueue count] > 0)
		{
			...
			if ([currentWrite isKindOfClass:[FWAsyncSpecialPacket class]])
			{
				LogVerbose(@"Dequeued FWAsyncSpecialPacket");
			...
			{
				LogVerbose(@"Dequeued FWAsyncWritePacket");
...
- (void)doWriteData
{
	...
	// Note: This method is not called if currentWrite is a FWAsyncSpecialPacket (startTLS packet)
...
- (void)doWriteTimeout
{
	...
	{
		FWAsyncWritePacket *theWrite = currentWrite;
...
- (void)startTLS:(NSDictionary *)tlsSettings
{
	...
	FWAsyncSpecialPacket *packet = [[FWAsyncSpecialPacket alloc] initWithTLSSettings:tlsSettings];
...
- (void)maybeStartTLS
{
	...
	if ((flags & kStartingReadTLS) && (flags & kStartingWriteTLS))
	{
		...
		#if TARGET_OS_IPHONE
		{
			FWAsyncSpecialPacket *tlsPacket = (FWAsyncSpecialPacket *)currentRead;
            NSDictionary *tlsSettings = @{};
            if (tlsPacket) {
                tlsSettings = tlsPacket->tlsSettings;
            }
			NSNumber *value = [tlsSettings objectForKey:FWAsyncSocketUseCFStreamForTLS];
...
- (OSStatus)sslReadWithBuffer:(void *)buffer length:(size_t *)bufferLength
{
	...
	{
		LogVerbose(@"%@ - No data available to read...", NSStringFromSelector(_cmd));
	...
	{
		LogVerbose(@"%@: Reading from SSL pre buffer...", NSStringFromSelector(_cmd));
		...
		LogVerbose(@"%@: Copying %zu bytes from sslPreBuffer", NSStringFromSelector(_cmd), bytesToCopy);
		
		memcpy(buffer, [sslPreBuffer readBuffer], bytesToCopy);
		[sslPreBuffer didRead:bytesToCopy];
		
		LogVerbose(@"%@: sslPreBuffer.length = %zu", NSStringFromSelector(_cmd), [sslPreBuffer availableBytes]);
		...
		if (done) LogVerbose(@"%@: Complete", NSStringFromSelector(_cmd));
	}
	...
	{
		LogVerbose(@"%@: Reading from socket...", NSStringFromSelector(_cmd));
		...
		if (socketFDBytesAvailable > totalBytesLeftToBeRead)
		{
			...
			LogVerbose(@"%@: Reading into sslPreBuffer...", NSStringFromSelector(_cmd));
		...
		else
		{
			...
			LogVerbose(@"%@: Reading directly into dataBuffer...", NSStringFromSelector(_cmd));
		...
		LogVerbose(@"%@: read from socket = %zd", NSStringFromSelector(_cmd), result);
		
		if (result < 0)
		{
			LogVerbose(@"%@: read errno = %i", NSStringFromSelector(_cmd), errno);
		...
		{
			LogVerbose(@"%@: read EOF", NSStringFromSelector(_cmd));
		...
		else
		{
			...
			if (readIntoPreBuffer)
			{
				...
				LogVerbose(@"%@: Copying %zu bytes out of sslPreBuffer", NSStringFromSelector(_cmd), bytesToCopy);
...
{
	FWAsyncSocket *asyncSocket = (__bridge FWAsyncSocket *)connection;
...
- (void)ssl_startTLS
{
	...
	//  3. FWAsyncSocketSSLPeerID
	//  4. FWAsyncSocketSSLProtocolVersionMin
	//  5. FWAsyncSocketSSLProtocolVersionMax
	//  6. FWAsyncSocketSSLSessionOptionFalseStart
	...
	value = [tlsSettings objectForKey:FWAsyncSocketSSLPeerID];
	...
	{
		NSAssert(NO, @"Invalid value for FWAsyncSocketSSLPeerID. Value must be of type NSData."
		             @" (You can convert strings to data using a method like"
		             @" [string dataUsingEncoding:NSUTF8StringEncoding])");
		
		[self closeWithError:[self otherError:@"Invalid value for FWAsyncSocketSSLPeerID."]];
		return;
	}
	
	// 4. FWAsyncSocketSSLProtocolVersionMin
	
	value = [tlsSettings objectForKey:FWAsyncSocketSSLProtocolVersionMin];
	...
	{
		NSAssert(NO, @"Invalid value for FWAsyncSocketSSLProtocolVersionMin. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for FWAsyncSocketSSLProtocolVersionMin."]];
		return;
	}
	
	// 5. FWAsyncSocketSSLProtocolVersionMax
	
	value = [tlsSettings objectForKey:FWAsyncSocketSSLProtocolVersionMax];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for FWAsyncSocketSSLProtocolVersionMax. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for FWAsyncSocketSSLProtocolVersionMax."]];
		return;
	}
	...
	{
		NSAssert(NO, @"Invalid value for FWAsyncSocketSSLSessionOptionFalseStart. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for FWAsyncSocketSSLSessionOptionFalseStart."]];
		return;
	}
	...
	{
		NSAssert(NO, @"Invalid value for FWAsyncSocketSSLSessionOptionSendOneByteRecord."
		             @" Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for FWAsyncSocketSSLSessionOptionSendOneByteRecord."]];
		return;
	}
	
	// 8. FWAsyncSocketSSLCipherSuites
	
	value = [tlsSettings objectForKey:FWAsyncSocketSSLCipherSuites];
	if ([value isKindOfClass:[NSArray class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for FWAsyncSocketSSLCipherSuites. Value must be of type NSArray.");
		
		[self closeWithError:[self otherError:@"Invalid value for FWAsyncSocketSSLCipherSuites."]];
		return;
	}
	
	// 9. FWAsyncSocketSSLDiffieHellmanParameters
	
	#if !TARGET_OS_IPHONE
	value = [tlsSettings objectForKey:FWAsyncSocketSSLDiffieHellmanParameters];
	if ([value isKindOfClass:[NSData class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for FWAsyncSocketSSLDiffieHellmanParameters. Value must be of type NSData.");
		
		[self closeWithError:[self otherError:@"Invalid value for FWAsyncSocketSSLDiffieHellmanParameters."]];
		return;
	}
	#endif
	...
	if (value)
	{
		NSAssert(NO, @"Security option unavailable - kCFStreamSSLLevel"
		             @" - You must use FWAsyncSocketSSLProtocolVersionMin & FWAsyncSocketSSLProtocolVersionMax");
	...
	sslPreBuffer = [[FWAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
- (void)ssl_continueSSLHandshake
{
	...
	else if (status == errSSLPeerAuthCompleted)
	{
		...
		else
		{
			...
			NSString *msg = @"FWAsyncSocketManuallyEvaluateTrust specified in tlsSettings,"
...
- (void)cf_startTLS
{
	...
	NSAssert([currentRead isKindOfClass:[FWAsyncSpecialPacket class]], @"Invalid read packet for startTLS");
	NSAssert([currentWrite isKindOfClass:[FWAsyncSpecialPacket class]], @"Invalid write packet for startTLS");
...
+ (void)startCFStreamThreadIfNeeded
{
	...
	dispatch_once(&predicate, ^{
		...
		cfstreamThreadSetupQueue = dispatch_queue_create("FWAsyncSocket-CFStreamThreadSetup", DISPATCH_QUEUE_SERIAL);
	});
...
+ (void)scheduleCFStreams:(FWAsyncSocket *)asyncSocket
{
	LogTrace();
...
+ (void)unscheduleCFStreams:(FWAsyncSocket *)asyncSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == cfstreamThread, @"Invoked on wrong thread");
...
{
	FWAsyncSocket *asyncSocket = (__bridge FWAsyncSocket *)pInfo;
...
{
	LogVerbose(@"%@ %@", NSStringFromSelector(_cmd), (includeReadWrite ? @"YES" : @"NO"));
...
- (int)socketFD
{
	if (!dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey))
	{
		LogWarn(@"%@ - Method only available from within the context of a performBlock: invocation", NSStringFromSelector(_cmd));
		return SOCKET_NULL;
	}
