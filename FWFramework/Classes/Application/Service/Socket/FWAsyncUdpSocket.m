...
@class FWAsyncUdpSendPacket;

NSString *const FWAsyncUdpSocketException = @"FWAsyncUdpSocketException";
NSString *const FWAsyncUdpSocketErrorDomain = @"FWAsyncUdpSocketErrorDomain";

NSString *const FWAsyncUdpSocketQueueName = @"FWAsyncUdpSocket";
NSString *const FWAsyncUdpSocketThreadName = @"FWAsyncUdpSocket-CFStream";

enum FWAsyncUdpSocketFlags
{
...
@interface FWAsyncUdpSocket ()
{
	...
	FWAsyncUdpSocketReceiveFilterBlock receiveFilterBlock;
	dispatch_queue_t receiveFilterQueue;
	BOOL receiveFilterAsync;
	
	FWAsyncUdpSocketSendFilterBlock sendFilterBlock;
	...
	FWAsyncUdpSendPacket *currentSend;
...
/**
 * The FWAsyncUdpSendPacket encompasses the instructions for a single send/write.
**/
@interface FWAsyncUdpSendPacket : NSObject {
@public
	NSData *buffer;
...
@interface FWAsyncUdpSpecialPacket : NSObject {
@public
//	uint8_t type;
...
- (instancetype)initWithDelegate:(id<FWAsyncUdpSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	...
	if ((self = [super init]))
	{
		...
		{
			socketQueue = dispatch_queue_create([FWAsyncUdpSocketQueueName UTF8String], NULL);
		}
...
- (void)setIPv4Enabled:(BOOL)flag
{
	...
	dispatch_block_t block = ^{
		...
		LogVerbose(@"%@ %@", NSStringFromSelector(_cmd), (flag ? @"YES" : @"NO"));
...
- (void)setMaxReceiveIPv4BufferSize:(uint16_t)max
{
	dispatch_block_t block = ^{
		...
		LogVerbose(@"%@ %u", NSStringFromSelector(_cmd), (unsigned)max);
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"FWAsyncUdpSocketSendTimeoutError",
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"FWAsyncUdpSocketClosedError",
	                                                     @"FWAsyncUdpSocket", [NSBundle mainBundle],
	                                                     @"Socket closed", nil);
...
- (BOOL)createSocket4:(BOOL)useIPv4 socket6:(BOOL)useIPv6 error:(NSError * __autoreleasing *)errPtr
{
	...
	int(^createSocket)(int) = ^int (int domain) {
         ...
         *
         * This is the reason of #222(FW does not necessarily return the size of an entire UDP packet) and
         *  #535(FWAsyncUDPSocket can not send data when data is greater than 9K)
         *
         *
...
- (BOOL)connectToHost:(NSString *)host onPort:(uint16_t)port error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		FWAsyncUdpSpecialPacket *packet = [[FWAsyncUdpSpecialPacket alloc] init];
...
- (void)maybeConnect
{
	...
	BOOL sendQueueReady = [currentSend isKindOfClass:[FWAsyncUdpSpecialPacket class]];
	
	if (sendQueueReady)
	{
		FWAsyncUdpSpecialPacket *connectPacket = (FWAsyncUdpSpecialPacket *)currentSend;
...
- (void)sendData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	FWAsyncUdpSendPacket *packet = [[FWAsyncUdpSendPacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (void)sendData:(NSData *)data toAddress:(NSData *)remoteAddr withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	FWAsyncUdpSendPacket *packet = [[FWAsyncUdpSendPacket alloc] initWithData:data timeout:timeout tag:tag];
	packet->addressFamily = [FWAsyncUdpSocket familyFromAddress:remoteAddr];
...
- (void)setSendFilter:(FWAsyncUdpSocketSendFilterBlock)filterBlock withQueue:(dispatch_queue_t)filterQueue
{
	[self setSendFilter:filterBlock withQueue:filterQueue isAsynchronous:YES];
}

- (void)setSendFilter:(FWAsyncUdpSocketSendFilterBlock)filterBlock
            withQueue:(dispatch_queue_t)filterQueue
       isAsynchronous:(BOOL)isAsynchronous
{
	FWAsyncUdpSocketSendFilterBlock newFilterBlock = NULL;
...
- (void)maybeDequeueSend
{
	...
	if (currentSend == nil)
	{
		...
		while ([sendQueue count] > 0)
		{
			...
			if ([currentSend isKindOfClass:[FWAsyncUdpSpecialPacket class]])
			{
				[self maybeConnect];
...
- (void)doPreSend
{
	...
	if (sendFilterBlock && sendFilterQueue)
	{
		...
		if (sendFilterAsync)
		{
			...
			FWAsyncUdpSendPacket *sendPacket = currentSend;
...
- (void)setReceiveFilter:(FWAsyncUdpSocketReceiveFilterBlock)filterBlock withQueue:(dispatch_queue_t)filterQueue
{
	[self setReceiveFilter:filterBlock withQueue:filterQueue isAsynchronous:YES];
}

- (void)setReceiveFilter:(FWAsyncUdpSocketReceiveFilterBlock)filterBlock
               withQueue:(dispatch_queue_t)filterQueue
          isAsynchronous:(BOOL)isAsynchronous
{
	FWAsyncUdpSocketReceiveFilterBlock newFilterBlock = NULL;
	dispatch_queue_t newFilterQueue = NULL;
...
- (void)doReceive
{
	...
	if (doReceive4)
	{
		...
		// #222: FW does not necessarily return the size of an entire UDP packet
...
+ (void)listenerThread:(id)unused
{
	@autoreleasepool {
		...
		[[NSThread currentThread] setName:FWAsyncUdpSocketThreadName];
...
+ (void)addStreamListener:(FWAsyncUdpSocket *)asyncUdpSocket
{
	LogTrace();
...
+ (void)removeStreamListener:(FWAsyncUdpSocket *)asyncUdpSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == listenerThread, @"Invoked on wrong thread");
...
{
	@autoreleasepool {
		FWAsyncUdpSocket *asyncUdpSocket = (__bridge FWAsyncUdpSocket *)pInfo;
