...
extension Data {
    ...
    public func fwUnarchiveObject() -> Any? { return (self as NSData).fwUnarchiveObject() }
    /// 保存对象归档
...
extension Date {
    ...
    public var fwStringValue: String { return (self as NSDate).fwStringValue }
    /// 转化为字符串，自定义格式和时区
    public func fwString(format: String?, timeZone: TimeZone? = nil) -> String { return (self as NSDate).fwString(withFormat: format, timeZone: timeZone) }
...
extension String {
    ...
    public func fwSize(font: UIFont, drawSize: CGSize = CGSize(width: CGFloat.greatestFiniteMagnitude, height: CGFloat.greatestFiniteMagnitude), attributes: [NSAttributedString.Key: Any]? = nil) -> CGSize { return (self as NSString).fwSize(with: font, draw: drawSize, attributes: attributes) }
    /// 格式化文件大小为".0K/.1M/.1G"
    ...
    public func fwMatchesRegex(_ regex: String) -> Bool { return (self as NSString).fwMatchesRegex(regex) }
}
