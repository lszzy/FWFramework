...
extension Encodable {
    public var fw: FWWrapper<Encodable> {
        return FWWrapper(self)
    }
}

extension FWWrapper where T : Encodable {
    public func encoded(using encoder: FWAnyEncoder = JSONEncoder()) throws -> Data {
        return try encoder.encode(self.base)
    }
}
