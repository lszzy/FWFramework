...
@implementation UIViewController (FWRootNavigationController)
...
- (FWRootNavigationController *)rt_navigationController
{
    UIViewController *vc = self;
    while (vc && ![vc isKindOfClass:[FWRootNavigationController class]]) {
        vc = vc.navigationController;
    }
    return (FWRootNavigationController *)vc;
}

@end

@interface NSArray<ObjectType> (FWRootNavigationController)
- (NSArray *)rt_map:(id(^)(ObjectType obj, NSUInteger index))block;
- (BOOL)rt_any:(BOOL(^)(ObjectType obj))block;
@end

@implementation NSArray (FWRootNavigationController)
...
@interface FWRootNavigationController () <UINavigationControllerDelegate, UIGestureRecognizerDelegate>
...
@interface FWContainerController ()
...
static inline UIViewController *RTSafeUnwrapViewController(UIViewController *controller) {
    if ([controller isKindOfClass:[FWContainerController class]]) {
        return ((FWContainerController *)controller).contentViewController;
    }
    return controller;
}
...
__attribute((overloadable)) static inline UIViewController *RTSafeWrapViewController(UIViewController *controller,
                                                                                     ...
                                                                                     NSString *backTitle) {
    if (![controller isKindOfClass:[FWContainerController class]]) {
        return [FWContainerController containerControllerWithController:controller
...
@implementation FWContainerController
                         ...
                         backTitle:(NSString *)backTitle
{
    ...
    if (self) {
        ...
        self.containerNavigationController = [[FWContainerNavigationController alloc] initWithNavigationBarClass:navigationBarClass
                                                                                                    toolbarClass:nil];
        if (yesOrNo) {
...
@interface UIViewController (FWContainerNavigationController)
@property (nonatomic, assign, readonly) BOOL rt_hasSetInteractivePop;
@end

@implementation UIViewController (FWContainerNavigationController)
...
@implementation FWContainerNavigationController
...
- (void)viewDidLayoutSubviews
{
    ...
    if ([self.parentViewController isKindOfClass:[FWContainerController class]] &&
        [self.parentViewController.parentViewController isKindOfClass:[FWRootNavigationController class]]) {
...
- (UITabBarController *)tabBarController
{
    ...
    FWRootNavigationController *navigationController = self.rt_navigationController;
    if (tabController) {
...
- (NSArray *)viewControllers
{
    if (self.navigationController) {
        if ([self.navigationController isKindOfClass:[FWRootNavigationController class]]) {
...
{
    self = [super initWithRootViewController:[[FWContainerController alloc] initWithContentController:rootViewController]];
    if (self) {
                    ...
                    animated:(BOOL)animated
{
    ...
    [FWRootNavigationController attemptRotationToDeviceOrientation];
