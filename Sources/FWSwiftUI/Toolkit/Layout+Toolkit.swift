...
extension View {
    ...
    public func frame(relativeWidth: CGFloat, relativeHeight: CGFloat? = nil, alignment: Alignment = .center) -> some View {
        ...
        if let relativeHeight = relativeHeight {
            height = UIScreen.fw_relativeValue(relativeHeight)
        }
        return frame(width: UIScreen.fw_relativeValue(relativeWidth), height: height, alignment: alignment)
    }
    
    /// 设置相对frame
    public func frame(relativeHeight: CGFloat, alignment: Alignment = .center) -> some View {
        return frame(height: UIScreen.fw_relativeValue(relativeHeight), alignment: alignment)
    }
    
    /// 设置指定边相对间距
    public func padding(_ edges: Edge.Set = .all, relativeLength: CGFloat) -> some View {
        padding(edges, UIScreen.fw_relativeValue(relativeLength))
    }
    
    /// 设置相对圆角
    public func cornerRadius(relativeRadius: CGFloat, antialiased: Bool = true) -> some View {
        cornerRadius(UIScreen.fw_relativeValue(relativeRadius), antialiased: antialiased)
    }
    
    /// 设置相对不规则圆角
    public func cornerRadius(relativeRadius: CGFloat, corners: UIRectCorner) -> some View {
        cornerRadius(UIScreen.fw_relativeValue(relativeRadius), corners: corners)
    }
    
    /// 同时设置相对边框和圆角
    public func border<S: ShapeStyle>(_ content: S, relativeWidth: CGFloat, relativeRadius: CGFloat) -> some View {
        border(content, width: UIScreen.fw_relativeValue(relativeWidth), cornerRadius: UIScreen.fw_relativeValue(relativeRadius))
    }
    
    /// 设置相对宽度边框
    public func border<S>(_ content: S, relativeWidth: CGFloat) -> some View where S : ShapeStyle {
        border(content, width: UIScreen.fw_relativeValue(relativeWidth))
    }
    ...
    public func shadow(color: Color = Color(.sRGBLinear, white: 0, opacity: 0.33), relativeRadius: CGFloat, relativeX: CGFloat = 0, relativeY: CGFloat = 0) -> some View {
        shadow(color: color, radius: UIScreen.fw_relativeValue(relativeRadius), x: UIScreen.fw_relativeValue(relativeX), y: UIScreen.fw_relativeValue(relativeY))
    }
...
extension HStack {
    ...
    public init(alignment: VerticalAlignment = .center, relativeSpacing: CGFloat, @ViewBuilder content: () -> Content) {
        self.init(alignment: alignment, spacing: UIScreen.fw_relativeValue(relativeSpacing), content: content)
    }
...
extension Spacer {
    ...
    public init(relativeMinLength: CGFloat) {
        self.init(minLength: UIScreen.fw_relativeValue(relativeMinLength))
    }
...
extension EdgeInsets {
    ...
    public init(_ edges: Edge.Set = .all, relativeLength: CGFloat) {
        ...
        if edges.contains(.top) { top = UIScreen.fw_relativeValue(relativeLength) }
        if edges.contains(.leading) { leading = UIScreen.fw_relativeValue(relativeLength) }
        if edges.contains(.bottom) { bottom = UIScreen.fw_relativeValue(relativeLength) }
        if edges.contains(.trailing) { trailing = UIScreen.fw_relativeValue(relativeLength) }
    }
...
extension LazyHStack {
    ...
    public init(alignment: VerticalAlignment = .center, relativeSpacing: CGFloat, pinnedViews: PinnedScrollableViews = .init(), @ViewBuilder content: () -> Content) {
        self.init(alignment: alignment, spacing: UIScreen.fw_relativeValue(relativeSpacing), pinnedViews: pinnedViews, content: content)
    }
...
extension LazyHGrid {
    ...
    public init(rows: [GridItem], alignment: VerticalAlignment = .center, relativeSpacing: CGFloat, pinnedViews: PinnedScrollableViews = .init(), @ViewBuilder content: () -> Content) {
        self.init(rows: rows, alignment: alignment, spacing: UIScreen.fw_relativeValue(relativeSpacing), pinnedViews: pinnedViews, content: content)
    }
...
extension LazyVGrid {
    ...
    public init(columns: [GridItem], alignment: HorizontalAlignment = .center, relativeSpacing: CGFloat, pinnedViews: PinnedScrollableViews = .init(), @ViewBuilder content: () -> Content) {
        self.init(columns: columns, alignment: alignment, spacing: UIScreen.fw_relativeValue(relativeSpacing), pinnedViews: pinnedViews, content: content)
    }
