...
open class WebSocket: WebSocketClient, WebSocketEngineDelegate, @unchecked Sendable {
    ...
    private func write(data: Data, opcode: WebSocketFrameOpCode, completion: (@Sendable () -> Void)?) {
...
public class WebSocketTCPTransport: WebSocketTransport, @unchecked Sendable {
    ...
    public func write(data: Data, completion: @escaping (@Sendable (Error?) -> Void)) {
...
public protocol WebSocketCertificatePinning: AnyObject {
    func evaluateTrust(trust: SecTrust, domain: String?, completion: (WebSocketPinningState) -> Void)
}
...
extension WebSocketFoundationSecurity: WebSocketCertificatePinning {
    public func evaluateTrust(trust: SecTrust, domain: String?, completion: (WebSocketPinningState) -> Void) {
        if allowSelfSigned {
            completion(.success)
            return
        }
    ...
    private func handleSecurityTrust(trust: SecTrust, completion: (WebSocketPinningState) -> Void) {
        var error: CFError?
...
public protocol WebSocketEngineProtocol {
    ...
    func forceStop()
    func write(data: Data, opcode: WebSocketFrameOpCode, completion: (@Sendable () -> Void)?)
    func write(string: String, completion: (@Sendable () -> Void)?)
}
    ...
    public func add(data: Data) {
        queue.async { [weak self] in
            ...
            while true {
                ...
                case .needsMoreData:
                    return
                case let .processedFrame(frame, split):
                    guard let s = self else { return }
...
extension Array where Element: WebSocketArrayType & UnsignedInteger {
    ...
    public func readUint16(offset: Int) -> UInt16 {
        (UInt16(self[offset + 0]) << 8) | UInt16(self[offset + 1])
    }
