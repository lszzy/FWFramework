...
@MainActor open class PopupMenuDeviceOrientationManager: NSObject {
    ...
    @objc func deviceOrientationDidChange(_ notify: Notification) {
        ...
        if let orientation, orientation != .unknown {
...
open class PopupMenu: UIView, UITableViewDataSource, UITableViewDelegate {
    ...
    open weak var containerView: UIView? {
        get {
            _containerView ?? UIWindow.fw.main
        }
        set {
    ...
    open func show() {
        ...
        animationManager.displayShowAnimationCompletion { [weak self] in
            ...
            delegate?.popupMenuDidShow?(self)
        }
    }
    ...
    open func dismiss() {
        ...
        animationManager.displayDismissAnimationCompletion { [weak self] in
            guard let self else { return }
            delegate?.popupMenuDidDismiss?(self)
            delegate = nil
            removeFromSuperview()
            menuMaskView.removeFromSuperview()
        }
    }
    ...
    private func updateUI() {
        ...
        if arrowDirection == .top {
            ...
            if arrowPosition > itemWidth / 2 {
                frame = CGRect(x: containerSize.width - minSpace - itemWidth, y: y, width: itemWidth, height: height + arrowHeight)
            } else if arrowPosition < itemWidth / 2 {
                frame = CGRect(x: minSpace, y: y, width: itemWidth, height: height + arrowHeight)
            } else {
                frame = CGRect(x: point.x - itemWidth / 2, y: y, width: itemWidth, height: height + arrowHeight)
            }
        } else if arrowDirection == .bottom {
            ...
            if arrowPosition < itemHeight / 2 {
                frame = CGRect(x: x, y: point.y - arrowPosition, width: itemWidth + arrowHeight, height: height)
