...
public class Asset: NSObject, @unchecked Sendable {
    ...
    public func previewImage(size: CGSize? = nil) -> UIImage? {
        ...
        var resultImage: UIImage?
        AssetManager.shared.phCachingImageManager.requestImage(for: phAsset, targetSize: size ?? CGSize(width: UIScreen.fw.screenWidth * 2, height: UIScreen.fw.screenHeight * 2), contentMode: .aspectFill, options: imageRequestOptions) { result, _ in
            resultImage = result
        }
        return resultImage
    }
    ...
    public func requestPreviewImage(
        ...
        let imageRequestId = AssetManager.shared.phCachingImageManager.requestImage(for: phAsset, targetSize: size ?? CGSize(width: UIScreen.fw.screenWidth * 2, height: UIScreen.fw.screenHeight * 2), contentMode: .aspectFill, options: imageRequestOptions) { @Sendable result, info in
            let downloadSucceed = (result != nil && info == nil) || (!Asset.isValueTrue(info, key: PHImageCancelledKey) && info?[PHImageErrorKey] == nil && !Asset.isValueTrue(info, key: PHImageResultIsDegradedKey))
            let downloadFailed = info?[PHImageErrorKey] != nil
            completion?(result, info, downloadSucceed || downloadFailed)
        }
    ...
    public func requestLivePhoto(
        ...
        let livePhotoRequestId = AssetManager.shared.phCachingImageManager.requestLivePhoto(for: phAsset, targetSize: size ?? CGSize(width: UIScreen.fw.screenWidth * 2, height: UIScreen.fw.screenHeight * 2), contentMode: .aspectFill, options: livePhotoRequestOptions) { @Sendable livePhoto, info in
...
open class AssetSessionExporter: NSObject, @unchecked Sendable {
    ...
    open func validateVideoOutputConfiguration() -> Bool {
        guard let videoOutputConfiguration else {
            return false
        }
...
extension AssetSessionExporter {
    ...
    public func export(renderHandler: RenderHandler? = nil,
        ...
        if _writer?.status == .writing {
            _writer?.cancelWriting()
            _writer = nil
        }

        if _reader?.status == .reading {
        ...
        if let _ = videoOutputConfiguration, validateVideoOutputConfiguration() == false {
        ...
        _completionHandler = completionHandler != nil ? { @MainActor @Sendable result in
            DispatchQueue.fw.mainAsync {
        ...
        setupVideoOutput(withAsset: asset)
        setupAudioOutput(withAsset: asset)
        setupAudioInput()
...
extension AssetSessionExporter {
    ...
    private func setupAudioInput() {
        ...
        if let writer = _writer, let audioInput = _audioInput {
...
extension AssetSessionExporter {
    ...
    func encode(readySamplesFromReaderOutput output: AVAssetReaderOutput, toWriterInput input: AVAssetWriterInput) -> Bool {
        while input.isReadyForMoreMediaData {
            guard _reader?.status == .reading && _writer?.status == .writing,
    ...
    func createVideoComposition() -> AVMutableVideoComposition {
    ...
    @MainActor func complete() {
        ...
        guard let reader = _reader else {
            _completionHandler?(.failure(AssetSessionExporterError.setupFailure))
        ...
        _completionHandler?(.success(status))
        _completionHandler = nil
    }
