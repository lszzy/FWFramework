...
extension Wrapper where Base: NSObject {
    ...
    private func messageTargets(_ lazyload: Bool) -> NSMutableDictionary? {
        var targets = objc_getAssociatedObject(base, &AssociatedKeys.messageTargets) as? NSMutableDictionary
        if targets == nil && lazyload {
            targets = NSMutableDictionary()
            objc_setAssociatedObject(base, &AssociatedKeys.messageTargets, targets, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
        return targets
    }
...
extension Wrapper where Base: NSObject {
    ...
    private func notificationTargets(_ lazyload: Bool) -> NSMutableDictionary? {
        var targets = objc_getAssociatedObject(base, &AssociatedKeys.notificationTargets) as? NSMutableDictionary
        if targets == nil && lazyload {
            targets = NSMutableDictionary()
            objc_setAssociatedObject(base, &AssociatedKeys.notificationTargets, targets, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
        return targets
    }
...
extension Wrapper where Base: NSObject {
    ...
    private func kvoTargets(_ lazyload: Bool) -> NSMutableDictionary? {
        var targets = objc_getAssociatedObject(base, &AssociatedKeys.kvoTargets) as? NSMutableDictionary
        if targets == nil && lazyload {
            targets = NSMutableDictionary()
            objc_setAssociatedObject(base, &AssociatedKeys.kvoTargets, targets, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
        return targets
    }
