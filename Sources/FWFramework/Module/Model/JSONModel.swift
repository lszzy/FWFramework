...
extension _Measurable {
    ...
    // TODO: 类型解析缓存
    func getProperties(_ children: [(String, Any)]? = nil) -> [PropertyInfo] {
        let instanceIsNsObject = isNSObjectType()
        let bridgedProperty = getBridgedPropertyList()
        
        let children = children ?? readAllChildrenFrom(mirror: Mirror(reflecting: self))
        return children.map { child in
            return PropertyInfo(key: child.0, type: type(of: child.1), bridged: instanceIsNsObject && bridgedProperty.contains(child.0))
        }
    }
...
fileprivate func getRawValueFrom(dict: [String: Any], property: PropertyInfo, mapper: HelpingMapper) -> Any? {
    if let mappingHandler = mapper.getMappingHandler(key: property.key) {
...
fileprivate func convertValue(rawValue: Any, property: PropertyInfo, mapper: HelpingMapper) -> Any? {
    ...
    if let mappingHandler = mapper.getMappingHandler(key: property.key), let transformer = mappingHandler.assignmentClosure {
...
fileprivate func assignProperty(convertedValue: Any, instance: _ExtendCustomModelType, property: PropertyInfo) {
    if property.bridged {
        ...
        extensions(of: property.type).write(convertedValue, to: instance, forKey: property.key)
    }
}
...
extension _ExtendCustomModelType {
    ...
    static func _transform(dict: [String: Any], to instance: inout Self) {
        let properties = instance.getProperties()
        ...
        for property in properties {
            if mapper.propertyExcluded(key: property.key) {
                InternalLogger.logDebug("Exclude property: \(property.key)")
                continue
            }

            InternalLogger.logVerbose("field: ", property.key, "  isBridgeProperty: ", property.bridged)

            if let rawValue = getRawValueFrom(dict: _dict, property: property, mapper: mapper) {
                if let convertedValue = convertValue(rawValue: rawValue, property: property, mapper: mapper) {
                    assignProperty(convertedValue: convertedValue, instance: instance, property: property)
                    continue
                }
            }
...
extension _ExtendCustomModelType {
    ...
    static func _serializeAny(object: _Transformable) -> Any? {
        ...
        case .class, .struct:
            ...
            let properties = object.getProperties(children)
            guard !properties.isEmpty else {
            ...
            let requiredInfo = merge(children: children, propertyInfos: properties)
    ...
    static func _serializeModelObject(instance: _ExtendCustomModelType, properties: [String: (Any, PropertyInfo?)], mapper: HelpingMapper) -> [String: Any] {
        ...
        for (key, property) in properties {
            ...
            if let info = property.1 {
                ...
                if mapper.propertyExcluded(key: info.key) {
                    continue
                }

                if let mappingHandler = mapper.getMappingHandler(key: info.key) {
...
extension AnyExtensions {
    ...
    public static func write(_ value: Any, to instance: _ExtendCustomModelType, forKey: String) {
...
public class HelpingMapper {
    ...
    public func specify<T>(property: inout T, key: String, converter: @escaping (String) -> T) {
        self.specify(property: &property, key: key, name: nil, converter: converter)
    }
    ...
    public func exclude(key: String) {
        self._exclude(key: key)
    }
    
    fileprivate func addCustomMapping(key: String, mappingInfo: MappingPropertyHandler) {
        self.mappingHandlers[key] = mappingInfo
    }
    
    fileprivate func _exclude(key: String) {
...
public func >>> (mapper: HelpingMapper, key: String) {
    mapper._exclude(key: key)
}
