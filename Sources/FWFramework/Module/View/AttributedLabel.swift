...
open class AttributedLabel: UIView {
    ...
    open var attributedText: NSAttributedString? {
        get { attributedString.copy() as? NSAttributedString }
        set {
            if let newValue {
    ...
    override open var intrinsicContentSize: CGSize {
        sizeThatFits(CGSize(width: bounds.width, height: CGFloat.greatestFiniteMagnitude))
    }
    ...
    private func numberOfDisplayedLines() -> Int {
        guard let textFrame else { return 0 }
    ...
    private func attributedStringForDraw() -> NSAttributedString {
        ...
        var fontLineHeight = font?.lineHeight ?? .zero
    ...
    private func urlForPoint(_ point: CGPoint) -> AttributedLabelURL? {
        ...
        guard bounds.insetBy(dx: 0, dy: -margin).contains(point), let textFrame else {
            return nil
        }
    ...
    private func transformForCoreText() -> CGAffineTransform {
        CGAffineTransform(translationX: 0, y: bounds.height).scaledBy(x: 1.0, y: -1.0)
    }
    ...
    private func drawHighlight(rect: CGRect) {
        guard let touchedLink,
              let highlightColor,
              let textFrame else { return }
    ...
    private func drawMaxMetric(runs: CFArray, xHeight: inout CGFloat, underlinePosition: inout CGFloat, lineThickness: inout CGFloat) {
        ...
        for i in 0..<CFArrayGetCount(runs) {
            ...
            if let attrs, let fontValue = attrs[.init(kCTFontAttributeName as String)] {
        ...
        for i in 0..<numberOfLines {
            ...
            for k in 0..<runCount {
                ...
                if i == numberOfLines - 1 && k >= runCount - 2 && lineBreakMode == .byTruncatingTail {
                    ...
                    if bounds.width - attatchmentRect.minX - attachmentWidth < minEllipsesWidth {
                        continue
                    }
                }
                ...
                if let image = content as? UIImage {
                    ...
                    let viewFrame = CGRect(x: attatchmentRect.origin.x, y: bounds.height - attatchmentRect.origin.y - attatchmentRect.size.height, width: attatchmentRect.size.width, height: attatchmentRect.size.height)
                    view.frame = viewFrame
                }
            }
        }
    ...
    private func computeLink(_ text: String) {
        ...
        linkDetector?.detectLinks(text, completion: { [weak self] links in
            ...
            if text == plainText {
                linkDetected = true
                if let links, links.count > 0 {
                    for link in links {
                        addAutoDetectedLink(link)
                    }
                    resetTextFrame()
                }
                ignoreRedraw = false
...
open class AttributedLabelURLDetector: NSObject, AttributedLabelURLDetectorProtocol, @unchecked Sendable {
    ...
    open func detectLinks(_ plainText: String, completion: ([AttributedLabelURL]?) -> Void) {
        ...
        for regularExpression in regularExpressions {
            ...
            regularExpression.enumerateMatches(in: plainText, range: NSMakeRange(0, plainString.length)) { result, _, _ in
