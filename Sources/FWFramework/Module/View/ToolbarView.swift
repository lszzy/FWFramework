...
open class ToolbarView: UIView {
    ...
    override open func updateConstraints() {
        ...
        let toolbarHidden = isHidden || toolbarHidden
...
open class ToolbarMenuView: UIView {
    ...
    open var titleView: ToolbarTitleView? {
        get {
            centerButton as? ToolbarTitleView
        }
        set {
    ...
    override open func updateConstraints() {
        ...
        if equalWidth {
            ...
            if let leftButton { subviewButtons.append(leftButton) }
            if let leftMoreButton { subviewButtons.append(leftMoreButton) }
            if let centerButton { subviewButtons.append(centerButton) }
            if let rightMoreButton { subviewButtons.append(rightMoreButton) }
            if let rightButton { subviewButtons.append(rightButton) }
        ...
        let leftButton = leftButton ?? leftMoreButton
        let leftMoreButton = self.leftButton != nil && leftMoreButton != nil ? leftMoreButton : nil
        if let leftButton {
        ...
        if let leftButton, let leftMoreButton {
        ...
        let rightButton = rightButton ?? rightMoreButton
        let rightMoreButton = self.rightButton != nil && rightMoreButton != nil ? rightMoreButton : nil
        if let rightButton {
        ...
        if let rightButton, let rightMoreButton {
...
open class ToolbarTitleView: UIControl, TitleViewProtocol {
    ...
    open var loadingViewSize: CGSize = .init(width: 18, height: 18)
    ...
    open var subAccessoryViewOffset: CGPoint = .init(x: 3, y: 0) {
        didSet { refreshLayout() }
    }
    ...
    override open func layoutSubviews() {
        ...
        if style == .vertical {
            ...
            if let loadingView, showsLoadingPlaceholder || !loadingViewHidden {
            ...
            if isSubtitleLabelShowing {
                ...
                if isSubAccessoryViewShowing, let subAccessoryView {
    ...
    private var loadingViewSpacingSizeIfNeedsPlaceholder: CGSize {
        CGSize(width: loadingViewSpacingSize.width * (showsLoadingPlaceholder ? 2 : 1), height: loadingViewSpacingSize.height)
    }
    ...
    private var subAccessorySpacingSizeIfNeedesPlaceholder: CGSize {
        CGSize(width: subAccessorySpacingSize.width * (showsSubAccessoryPlaceholder ? 2 : 1), height: subAccessorySpacingSize.height)
    }
    ...
    private var subtitleEdgeInsetsIfShowingSubtitleLabel: UIEdgeInsets {
        (subtitleLabelSize.width <= 0 || subtitleLabelSize.height <= 0) ? .zero : subtitleEdgeInsets
    }
    ...
    private var secondLineWidthInVerticalStyle: CGFloat {
        ...
        if subtitleLabelSize.width > 0, let subAccessoryView, !subAccessoryView.isHidden {
...
open class ToolbarButton: UIButton {
    ...
    override open func setImage(_ image: UIImage?, for state: UIControl.State) {
        ...
        if let image, self.image(for: state) != image {
            if state == .normal {
                highlightedImage = image.fw.image(alpha: 0.2)?.withRenderingMode(image.renderingMode)
                setImage(highlightedImage, for: .highlighted)
                disabledImage = image.fw.image(alpha: 0.2)?.withRenderingMode(image.renderingMode)
                setImage(disabledImage, for: .disabled)
            } else {
                if image != highlightedImage && image != disabledImage {
                    if self.image(for: .highlighted) == highlightedImage && state != .highlighted {
                        setImage(nil, for: .highlighted)
                    }
                    if self.image(for: .disabled) == disabledImage && state != .disabled {
...
open class ExpandedTitleView: UIView {
    ...
    open weak var contentView: UIView? {
        didSet {
            ...
            oldValue?.removeFromSuperview()
            if let contentView, contentView.superview == nil {
                addSubview(contentView)
