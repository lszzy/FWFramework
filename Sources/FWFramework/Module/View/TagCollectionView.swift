...
open class TagCollectionView: UIView {
    ...
    open var contentInset: UIEdgeInsets = UIEdgeInsets(top: 2, left: 2, bottom: 2, right: 2) {
        didSet { setNeedsLayoutTagViews() }
    }
    ...
    @objc private func onTapGesture(_ tapGesture: UITapGestureRecognizer) {
        ...
        guard let dataSource = dataSource, let delegate = delegate,
    ...
    private func layoutTagViewsForVerticalDirection() {
        ...
        let totalWidth = manualCalculateHeight && preferredMaxLayoutWidth > 0 ? preferredMaxLayoutWidth : self.bounds.width
        let maxLineWidth = totalWidth - contentInset.left - contentInset.right
        var currentLineTagsCount: Int = 0
    ...
    private func layoutEachLineTags(
        ...
        for currentLine in 0..<numberOfLines {
            ...
            eachLineTagIndexs[currentLine].enumerated().forEach { (idx, tagIndex) in
...
open class TextTagConfig: NSObject, NSCopying {
    ...
    open var gradientBackgroundStartPoint: CGPoint = CGPoint(x: 0.5, y: 0)
    open var gradientBackgroundEndPoint: CGPoint = CGPoint(x: 0.5, y: 1.0)
    ...
    open var extraSpace: CGSize = CGSize(width: 14, height: 14)
...
open class TextTagCollectionView: UIView, TagCollectionViewDataSource, TagCollectionViewDelegate {
    ...
    open override func layoutSubviews() {
        ...
        if tagCollectionView.frame != self.bounds {
            updateAllLabelStyleAndFrame()
            tagCollectionView.frame = self.bounds
    ...
    private func updateStyleAndFrame(for label: TextTagLabel) {
        ...
        if scrollDirection == .vertical && self.bounds.width > 0 {
            maxSize.width = self.bounds.width - contentInset.left - contentInset.right
        }
