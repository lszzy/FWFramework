...
open class ZoomImageView: UIView, UIScrollViewDelegate, UIGestureRecognizerDelegate {
    ...
    open weak var videoPlayerItem: AVPlayerItem? {
        didSet {
            ...
            _videoPlayerLayer?.player = videoPlayer
            // 更新 videoPlayerView 的大小时，videoView 可能已经被缩放过，所以要应用当前的缩放
            videoPlayerView?.fw.frameApplyTransform = CGRect(x: 0, y: 0, width: videoSize.width, height: videoSize.height)
    ...
    open lazy var progressView: UIView & ProgressViewPlugin = {
        ...
        addSubview(result)
        (result as UIView).fw.alignCenter(autoScale: false)
        return result
    }() {
        didSet {
            ...
            (progressView as UIView).fw.alignCenter(autoScale: false)
        }
    }
    ...
    open func revertZooming() {
        ...
        let minimumZoomScale = minimumZoomScale
        let maximumZoomScale = enabledZoomImageView ? maximumZoomScale : minimumZoomScale
    ...
    private func configVideoProgressSlider() {
        guard let videoPlayerItem else { return }
    ...
    private func addPlayerTimeObserver() {
        ...
        videoTimeObserver = videoPlayer?.addPeriodicTimeObserver(forInterval: CMTimeMakeWithSeconds(0.1, preferredTimescale: Int32(NSEC_PER_SEC)), queue: nil, using: { [weak self] _ in
    ...
    private func timeString(from seconds: Double) -> String {
        let min = UInt(floor(seconds / 60.0))
        let sec = UInt(floor(seconds - Double(min * 60)))
    ...
    @objc private func handleDraggingVideoSlider(_ slider: UISlider) {
        ...
        let currentValue = Float64(slider.value)
...
open class ZoomImageVideoToolbar: UIView {
    ...
    override open func layoutSubviews() {
        ...
        sliderRightLabel.frame = CGRect(x: bounds.width - paddings.right - timeLabelWidth, y: paddings.top, width: timeLabelWidth, height: contentHeight)
