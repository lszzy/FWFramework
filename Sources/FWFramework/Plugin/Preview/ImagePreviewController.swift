...
open class ImagePreviewView: UIView, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, ZoomImageViewDelegate {
    ...
    open func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        customZoomImageView?(zoomView, indexPath.item)

        if delegate?.imagePreviewView?(self, renderZoomImageView: zoomView, at: indexPath.item) != nil {
        } else if let renderBlock = renderZoomImageView {
            renderBlock(zoomView, indexPath.item)
        } else if (imageURLs?.count ?? 0) > indexPath.item {
            let placeholderImage = placeholderImage?(indexPath.item)
    ...
    open func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if !isFirstDidScroll && (turnPageToRight || turnPageToLeft) {
            ...
            if roundIndex >= 0 && roundIndex < collectionView.numberOfItems(inSection: 0) {
...
private class ImagePreviewCell: UICollectionViewCell {
...
open class ImagePreviewController: UIViewController, UIViewControllerTransitioningDelegate {
    ...
    override open func viewDidLayoutSubviews() {
        ...
        let pageLabelCenter = pageLabelCenter?() ?? CGPoint(x: UIScreen.main.bounds.width / 2, y: UIScreen.main.bounds.height - (UIScreen.fw.safeAreaInsets.bottom + 18))
    ...
    open func dismissingGestureChanged(_ isHidden: Bool) {
        ...
        if let zoomImageView, zoomImageView.videoPlayerItem != nil {
...
open class ImagePreviewTransitionAnimator: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    open func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if isPresenting {
            ...
            if let fromView, let toView {
                containerView.insertSubview(toView, belowSubview: fromView)
            }
            presentingViewController?.beginAppearanceTransition(true, animated: true)
        }

        if let zoomImageView {
        ...
        if style == .scale {
            ...
            for attributes in resultAttributes {
                ...
                if distance >= distanceForMinimumScale {
                    ...
                    scale = minimumScale + (distanceForMinimumScale - distance) * (maximumScale - minimumScale) / (distanceForMinimumScale - distanceForMaximumScale)
                }
