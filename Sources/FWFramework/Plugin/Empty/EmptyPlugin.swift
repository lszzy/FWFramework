...
@MainActor extension Wrapper where Base: UIView {
    ...
    /// 显示错误空界面
    public func showEmptyView(error: Error?, action: AttributedStringParameter? = nil, block: (@MainActor @Sendable (Any) -> Void)? = nil) {
        showEmptyView(
    ...
    public func showEmptyView(text: AttributedStringParameter? = nil, detail: AttributedStringParameter? = nil, image: UIImage? = nil, action: AttributedStringParameter? = nil, block: (@MainActor @Sendable (Any) -> Void)? = nil) {
    ...
    public func showEmptyView(text: AttributedStringParameter?, detail: AttributedStringParameter?, image: UIImage?, loading: Bool, actions: [AttributedStringParameter]?, block: (@MainActor @Sendable (Int, Any) -> Void)?, customBlock: (@MainActor (Any) -> Void)? = nil) {
        ...
        if let scrollView = base as? UIScrollView {
            ...
            plugin.showEmptyView(text: text?.attributedStringValue, detail: detail?.attributedStringValue, image: image, loading: loading, actions: actions?.map(\.attributedStringValue), block: block, customBlock: customBlock, in: scrollView.fw.overlayView)
        } else {
            plugin.showEmptyView(text: text?.attributedStringValue, detail: detail?.attributedStringValue, image: image, loading: loading, actions: actions?.map(\.attributedStringValue), block: block, customBlock: customBlock, in: base)
        }
    }
...
@MainActor extension Wrapper where Base: UIViewController {
    /// 自定义空界面插件，未设置时自动从插件池加载
    public var emptyPlugin: EmptyPlugin! {
        get { base.view.fw.emptyPlugin }
        set { base.view.fw.emptyPlugin = newValue }
    }

    /// 设置空界面外间距，默认zero
    public var emptyInsets: UIEdgeInsets {
        get { base.view.fw.emptyInsets }
        set { base.view.fw.emptyInsets = newValue }
    }

    /// 获取正在显示的空界面视图
    public var showingEmptyView: UIView? {
        base.view.fw.showingEmptyView
    }

    /// 是否显示空界面
    public var hasEmptyView: Bool {
        base.view.fw.hasEmptyView
    }
...
@MainActor extension Wrapper where Base: UIScrollView {
    ...
    public weak var emptyViewDelegate: EmptyViewDelegate? {
        get {
            property(forName: "emptyViewDelegate") as? EmptyViewDelegate
        }
        set {
    ...
    public func reloadEmptyView() {
        guard let emptyViewDelegate else { return }
...
extension EmptyPlugin {
    ...
    public func showingEmptyView(in view: UIView) -> UIView? {
        EmptyPluginImpl.shared.showingEmptyView(in: view)
    }
}
...
extension FrameworkAutoloader {
    private nonisolated(unsafe) static var swizzleEmptyPluginFinished = false
