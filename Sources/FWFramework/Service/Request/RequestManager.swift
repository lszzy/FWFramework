...
open class RequestManager: @unchecked Sendable {
    ...
    open func incompleteDownloadTempPath(for request: HTTPRequest, downloadPath: String?) -> URL? {
        guard let downloadPath, !downloadPath.isEmpty else {
            return nil
        }
            ...
            if let requestRetrier = request.config.requestRetrier {
                ...
                requestRetrier.retryRequest(request, response: response, responseObject: responseObject, error: error) { [weak self] shouldRetry in
                    if shouldRetry {
                        ...
                        completionHandler?(response, sendableResponseObject.value, error)
                    }
                }
            } else {
    ...
    private func validateResumeData(_ data: Data?) -> Bool {
        ...
        if let resumeDictionary, !resumeDictionary.isEmpty {
            return true
        }
        return false
    }
