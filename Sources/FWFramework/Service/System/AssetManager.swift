...
public class Asset: NSObject {
    ...
    public func previewImage(size: CGSize? = nil) -> UIImage? {
        ...
        var resultImage: UIImage?
        AssetManager.shared.phCachingImageManager.requestImage(for: phAsset, targetSize: size ?? CGSize(width: UIScreen.main.bounds.width * 2, height: UIScreen.main.bounds.height * 2), contentMode: .aspectFill, options: imageRequestOptions) { result, _ in
            resultImage = result
        }
        return resultImage
    }
     ...
     */
    @discardableResult
    public func requestPreviewImage(size: CGSize? = nil, completion: ((_ result: UIImage?, _ info: [AnyHashable: Any]?, _ finished: Bool) -> Void)?, progressHandler: PHAssetImageProgressHandler? = nil) -> Int {
        let imageRequestOptions = PHImageRequestOptions()
        imageRequestOptions.isNetworkAccessAllowed = true
        imageRequestOptions.progressHandler = progressHandler
        
        let imageRequestId = AssetManager.shared.phCachingImageManager.requestImage(for: phAsset, targetSize: size ?? CGSize(width: UIScreen.main.bounds.width * 2, height: UIScreen.main.bounds.height * 2), contentMode: .aspectFill, options: imageRequestOptions) { result, info in
            let downloadSucceed = (result != nil && info == nil) || (!Asset.isValueTrue(info, key: PHImageCancelledKey) && info?[PHImageErrorKey] == nil && !Asset.isValueTrue(info, key: PHImageResultIsDegradedKey))
            let downloadFailed = info?[PHImageErrorKey] != nil
            completion?(result, info, downloadSucceed || downloadFailed)
        }
     ...
     */
    @discardableResult
    public func requestLivePhoto(size: CGSize? = nil, completion: ((_ livePhoto: PHLivePhoto?, _ info: [AnyHashable: Any]?, _ finished: Bool) -> Void)?, progressHandler: PHAssetImageProgressHandler? = nil) -> Int {
        ...
        let livePhotoRequestId = AssetManager.shared.phCachingImageManager.requestLivePhoto(for: phAsset, targetSize: size ?? CGSize(width: UIScreen.main.bounds.width * 2, height: UIScreen.main.bounds.height * 2), contentMode: .aspectFill, options: livePhotoRequestOptions) { livePhoto, info in
