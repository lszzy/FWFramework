...
public class VersionManager: @unchecked Sendable {
    ...
    public func openAppStore(force: Bool = false) {
        guard let appId, let storeUrl = URL(string: "https://apps.apple.com/app/id\(appId)") else { return }
    ...
    public func migrateData(_ completion: (@MainActor @Sendable () -> Void)?) -> Bool {
        ...
        let versions = dataMigrations.keys.sorted { str1, str2 in
            str1.compare(str2, options: .numeric) == .orderedAscending
        }
    ...
    private func parseResponse(_ data: Data, completion: (@MainActor @Sendable () -> Void)?) {
        ...
        // 请求成功更新检查日期
        checkDate = toCheckDate()
        UserDefaults.standard.set(checkDate, forKey: "FWVersionManagerCheckDate")
        UserDefaults.standard.synchronize()
    ...
    private func checkDataVersion(_ version: String) -> Bool {
        ...
        // 第一次需要执行
        guard let dataVersion else { return true }
        // 当前数据版本小于指定版本需要执行
