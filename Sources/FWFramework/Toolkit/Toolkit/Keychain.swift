...
public class KeychainManager: @unchecked Sendable {
    ...
    private func query(forService service: String?, account: String?) -> [String: Any] {
        ...
        if let service { query[String(kSecAttrService)] = service }
        if let account { query[String(kSecAttrAccount)] = account }
        if let group { query[String(kSecAttrAccessGroup)] = group }
        return query
    }
}
