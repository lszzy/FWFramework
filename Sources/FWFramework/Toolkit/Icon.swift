...
open class Icon: NSObject {
    ...
    open class func iconNamed(_ name: String, size: CGFloat) -> Icon? {
        ...
        guard let iconClass = iconClass else { return nil }
    ...
    open class func iconImage(_ name: String, size: CGFloat) -> UIImage? {
        return iconNamed(name, size: size)?.image
    }
    ...
    open class func installIconFont(_ fileURL: URL) -> Bool {
        ...
        guard let dataProvider = CGDataProvider(url: fileURL as CFURL),
              let newFont = CGFont.init(dataProvider) else { return false }
    ...
    open var backgroundColor: UIColor? {
        get {
            return attribute(.backgroundColor) as? UIColor
        }
        set {
    ...
    open var foregroundColor: UIColor? {
        get {
            return attribute(.foregroundColor) as? UIColor
        }
        set {
            if let color = newValue {
    ...
    open var iconFont: UIFont {
        return (attribute(.font) as? UIFont) ?? Self.iconFont(size: UIFont.systemFontSize)
    }
    ...
    open func attributes() -> [NSAttributedString.Key: Any] {
        return mutableAttributedString.attributes(at: 0, effectiveRange: nil)
    }
    
    /// 返回图标指定属性
    open func attribute(_ key: NSAttributedString.Key) -> Any? {
        return mutableAttributedString.attribute(key, at: 0, effectiveRange: nil)
    }
