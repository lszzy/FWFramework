...
@MainActor private class KeyboardTarget<T: UIView & UITextInput>: NSObject {
    ...
    lazy var toolbarPreviousButton: Any? = KeyboardConfig.shared.toolbarPreviousImage

    lazy var toolbarNextButton: Any? = KeyboardConfig.shared.toolbarNextImage
    ...
    @objc private func editingDidBegin() {
        ...
        if KeyboardConfig.shared.keyboardGesture == nil {
            KeyboardConfig.shared.keyboardGesture = UITapGestureRecognizer.fw.gestureRecognizer(block: { sender in
                if sender.state == .ended {
                    sender.view?.endEditing(true)
                }
            })
        }
        if let keyboardGesture = KeyboardConfig.shared.keyboardGesture {
    ...
    @objc private func keyboardWillShow(_ notification: Notification) {
        ...
        if let scrollView {
            if !KeyboardConfig.shared.keyboardShowing {
                KeyboardConfig.shared.keyboardShowing = true
                KeyboardConfig.shared.keyboardOffset = scrollView.contentOffset.y
            }
            ...
            var targetOffsetY = max(contentOffset.y + textInputOffset + CGRectGetMaxY(convertRect) - CGRectGetMinY(keyboardRect), KeyboardConfig.shared.keyboardOffset)
        ...
        if !KeyboardConfig.shared.keyboardShowing {
            KeyboardConfig.shared.keyboardShowing = true
            KeyboardConfig.shared.keyboardOrigin = viewController.view.frame.origin.y
        }
        ...
        let textInputOffset = keyboardDistanceBlock?(keyboardRect.height, convertRect.height) ?? keyboardDistance
        var viewTargetY = min(viewFrame.origin.y - textInputOffset + CGRectGetMinY(keyboardRect) - CGRectGetMaxY(convertRect), KeyboardConfig.shared.keyboardOrigin)
    ...
    @objc private func keyboardWillHide(_ notification: Notification) {
        ...
        guard keyboardManager, let viewController,
              KeyboardConfig.shared.keyboardShowing else { return }
        ...
        if let scrollView {
            let originOffsetY = KeyboardConfig.shared.keyboardOffset
            KeyboardConfig.shared.keyboardShowing = false
            KeyboardConfig.shared.keyboardOffset = 0
        ...
        let viewOriginY = KeyboardConfig.shared.keyboardOrigin
        KeyboardConfig.shared.keyboardShowing = false
        KeyboardConfig.shared.keyboardOrigin = 0
...
private class KeyboardConfig: @unchecked Sendable {
    ...
    var keyboardGesture: UITapGestureRecognizer?

    var toolbarPreviousImage: UIImage? {
