...
@MainActor extension Wrapper where Base: UIView {
    ...
    public var dragEnabled: Bool {
        get { return dragGesture.isEnabled }
...
extension UIView {
    ...
    @objc fileprivate func innerDragHandler(_ sender: UIPanGestureRecognizer) {
        ...
        var newOriginX = CGRectGetMinX(self.frame) + (fw.dragHorizontal ? translation.x : 0)
        var newOriginY = CGRectGetMinY(self.frame) + (fw.dragVertical ? translation.y : 0)
