...
class TestTableController: UIViewController, TableViewControllerProtocol {
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        tableView.app.height(cellClass: TestTableDynamicLayoutCell.self /* , cacheBy: indexPath */ ) { [weak self] cell in
    ...
    @objc func onRefreshing() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1) { [weak self] in
            ...
            tableData.removeAll()
            for _ in 0..<5 {
                tableData.append(randomObject())
            }
            tableView.app.clearHeightCache()
            tableView.reloadData()

            tableView.app.shouldRefreshing = tableData.count < 50
            tableView.app.endRefreshing()
            if !tableView.app.shouldRefreshing {
    ...
    @objc func onLoading() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1) { [weak self] in
            ...
            tableView.app.loadingFinished = tableData.count >= 50
    ...
    func onPhotoBrowser(_ cell: TestTableDynamicLayoutCell, indexPath: IndexPath) {
        ...
        app.showImagePreview(imageURLs: pictureUrls, imageInfos: nil, currentIndex: indexPath.row) { [weak self] index in
            ...
            if let cell = tableView.cellForRow(at: IndexPath(row: index, section: 0)) as? TestTableDynamicLayoutCell {
                return cell.myImageView
            } else {
                let cellFrame = tableView.app.layoutCellFrame(for: IndexPath(row: index, section: 0))
                let windowFrame = tableView.convert(cellFrame, to: view.window)
