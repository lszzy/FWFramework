...
@interface TestThreadController () <FWTableViewController>
...
{
    [self fw_showAlertWithTitle:@"结果" message:[NSString stringWithFormat:@"期望：%@\n实际：%@", @([self queueCount]), @(count)] cancel:nil cancelBlock:nil];
}
...
- (void)onRandom1
{
    ...
    [self onQueue:^{
        ...
        // 操作
        [self fw_lock];
        NSInteger value = [[dict.allKeys fw_randomObject:dict.allValues] integerValue];
        if (value == 1) {
            count1 += 1;
        } else if (value == 2) {
            count2 += 1;
