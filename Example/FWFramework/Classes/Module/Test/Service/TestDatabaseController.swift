...
class TestDatabaseModel: NSObject, DatabaseModel {
    // 用于模拟数据库结构更新
    static let isLatest = SendableObject(false)
...
class TestDatabaseController: UIViewController, TableViewControllerProtocol {
    ...
    func didInitialize() {
        ...
        TestDatabaseModel.isLatest.object = version > 1
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cods = !model.codings.isEmpty ? " - [\(model.codings.map(\.tag).joined(separator: ","))]" : ""
        let arc = model.archive != nil ? " - [\(model.archive?.tag ?? "")]" : ""
        let arcs = !model.archives.isEmpty ? " - [\(model.archives.map(\.tag).joined(separator: ","))]" : ""
    ...
    func onUpdate() {
        ...
        TestDatabaseModel.isLatest.object = true
    ...
    func onDelete() {
        ...
        TestDatabaseModel.isLatest.object = false
