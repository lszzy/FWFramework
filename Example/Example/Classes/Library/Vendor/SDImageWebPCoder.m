...
@implementation FWImageWebPCoder {
    WebPIDecoder *_idec;
    WebPDemuxer *_demux;
...
+ (instancetype)sharedCoder {
    static FWImageWebPCoder *coder;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        coder = [[FWImageWebPCoder alloc] init];
    });
    return coder;
}
...
- (BOOL)canDecodeFromData:(nullable NSData *)data {
    return ([NSData fw_imageFormatForImageData:data] == FWImageFormatWebP);
}
...
- (UIImage *)decodedImageWithData:(NSData *)data options:(nullable FWImageCoderOptions *)options {
    ...
    CGSize scaledSize = CGSizeMake(canvasWidth, canvasHeight);
    ...
    if (!hasAnimation || decodeFirstFrame) {
        ...
        firstFrameImage.fw_imageFormat = FWImageFormatWebP;
    ...
    CGContextRef canvas = CGBitmapContextCreate(NULL, canvasWidth, canvasHeight, 8, 0, [FWImageCoderHelper colorSpaceGetDeviceRGB], bitmapInfo);
    if (!canvas) {
        WebPDemuxDelete(demuxer);
        CGColorSpaceRelease(colorSpace);
        return nil;
    ...
    UIImage *animatedImage = [FWImageCoderHelper animatedImageWithFrames:frames];
    animatedImage.fw_imageLoopCount = loopCount;
    animatedImage.fw_imageFormat = FWImageFormatWebP;
...
- (UIImage *)incrementalDecodedImageWithOptions:(FWImageCoderOptions *)options {
    ...
    if (_demux) {
        ...
        SD_UNLOCK(_lock);
        image.fw_imageFormat = FWImageFormatWebP;
    ...
    if (width + height > 0 && last_y > 0 && height >= last_y) {
        ...
        CGColorSpaceRef colorSpaceRef = [FWImageCoderHelper colorSpaceGetDeviceRGB];
        ...
        CGContextRef canvas = CGBitmapContextCreate(NULL, width, height, 8, 0, [FWImageCoderHelper colorSpaceGetDeviceRGB], bitmapInfo);
        if (!canvas) {
            CGImageRelease(imageRef);
            return nil;
        }
        ...
        image.fw_isDecoded = YES; // Already drawn on bitmap context above
        image.fw_imageFormat = FWImageFormatWebP;
...
- (nonnull CGColorSpaceRef)sd_createColorSpaceWithDemuxer:(nonnull WebPDemuxer *)demuxer CF_RETURNS_RETAINED {
    ...
    if (!colorSpaceRef) {
        colorSpaceRef = [FWImageCoderHelper colorSpaceGetDeviceRGB];
...
- (BOOL)canEncodeToFormat:(FWImageFormat)format {
    return (format == FWImageFormatWebP);
}
...
- (NSData *)encodedDataWithImage:(UIImage *)image format:(FWImageFormat)format options:(nullable FWImageCoderOptions *)options {
    ...
    if (encodeFirstFrame || frames.count == 0) {
        ...
        for (size_t i = 0; i < frames.count; i++) {
            FWImageFrame *currentFrame = frames[i];
        ...
        int loopCount = (int)image.fw_imageLoopCount;
...
- (nullable NSData *)sd_encodedWebpDataWithImage:(nullable CGImageRef)imageRef quality:(double)quality {
    ...
    if (byteOrderNormal && ((alphaInfo == kCGImageAlphaNone) || (alphaInfo == kCGImageAlphaLast))) {
        ...
        vImage_CGImageFormat destFormat = {
            ...
            .colorSpace = [FWImageCoderHelper colorSpaceGetDeviceRGB],
...
- (instancetype)initWithAnimatedImageData:(NSData *)data options:(nullable FWImageCoderOptions *)options {
    if (!data) {
        return nil;
    }
    if (self) {
...
- (UIImage *)safeStaticImageFrame {
    ...
    if (_hasAnimation) {
        ...
        if (!_canvas) {
            ...
            CGContextRef canvas = CGBitmapContextCreate(NULL, _canvasWidth, _canvasHeight, 8, 0, [FWImageCoderHelper colorSpaceGetDeviceRGB], bitmapInfo);
            if (!canvas) {
                return nil;
            }
            _canvas = canvas;
        }
        CGSize scaledSize = CGSizeMake(_canvasWidth, _canvasHeight);
        imageRef = [self sd_drawnWebpImageWithCanvas:_canvas iterator:iter colorSpace:_colorSpace scaledSize:scaledSize];
    } else {
        CGSize scaledSize = CGSizeMake(iter.width, iter.height);
